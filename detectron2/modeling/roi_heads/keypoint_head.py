...
def keypoint_rcnn_focalloss(pred_keypoint_logits, instances, normalizer):
    """
    Arguments:
        pred_keypoint_logits (Tensor): A tensor of shape (N, K, S, S) where N is the total number
            of instances in the batch, K is the number of keypoints, and S is the side length
    ...
    input_tmp = pred_keypoint_logits[valid]
    tareget_tmp = nn.functional.one_hot(keypoint_targets[valid], input_tmp.size()[1]).to(torch.float32)
    keypoint_loss = torchvision.ops.sigmoid_focal_loss(input_tmp, tareget_tmp, reduction="sum")
...
def keypoint_rcnn_sigmoidloss(pred_keypoint_logits, instances, normalizer):
    """
    Arguments:
        pred_keypoint_logits (Tensor): A tensor of shape (N, K, S, S) where N is the total number
            of instances in the batch, K is the number of keypoints, and S is the side length
    ...
    input_tmp = pred_keypoint_logits[valid]
    tareget_tmp = nn.functional.one_hot(keypoint_targets[valid], input_tmp.size()[1]).to(torch.float32)
    keypoint_loss = F.binary_cross_entropy_with_logits(input_tmp, tareget_tmp, reduction="sum")
...
class BaseKeypointRCNNHead(nn.Module):
    ...
    @configurable
    def __init__(self, *, num_keypoints, loss_weight=1.0, loss_normalizer=1.0, loss_type='normal'):
        """
        ...
        assert loss_type in ['normal', 'focal', 'sigmoid'], loss_type
        loss_func_map = {
            'normal' : keypoint_rcnn_loss,
            'sigmoid' : keypoint_rcnn_sigmoidloss,
            'focal' : keypoint_rcnn_focalloss,
        }
        self.loss_func = loss_func_map[loss_type]
    ...
    def forward(self, x, instances: List[Instances]):
        ...
        if self.training:
            ...
            return {
                "loss_keypoint": self.loss_func(x, instances, normalizer=normalizer)
                * self.loss_weight
            }
        else:
